...
const CustomerInfoDatas: NextPage = () => {
  ...
  const limitCounnt = useCustomerStore((state) => state.limitCount);
  const setLimitCounnt = useCustomerStore((state) => state.setLimitCount);
  ...
  useEffect(() => {
    const getCustomerInfomations = async () => {
      ...
      const q = query(collectionRef, orderBy("createdAt", "desc"), limit(limitCounnt));
